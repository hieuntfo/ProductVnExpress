...
const App: React.FC = () => {
  ...
  const handleAddProject = (e: React.FormEvent) => {
    ...
    alert(`Đã lưu dự án ${projectToAdd.code} thành công!`);
  };
        ...
        <header className="flex items-center justify-between mb-8 border-b border-slate-200 pb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black uppercase text-[#9f224e] tracking-[0.2em]">Hệ thống VnExpress P&T</span>
            </div>
            ...
            <div className="flex bg-slate-200 p-1 rounded-xl mt-4 inline-flex">
              {[2025, 2026].map(yr => (
                <button 
                  key={yr}
          ...
          <div className="flex gap-4">
            <button onClick={() => window.location.reload()} className="p-3 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-[#9f224e] transition-all shadow-sm">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            </button>
            <button onClick={() => setIsAddingProject(true)} className="bg-[#9f224e] text-white px-6 py-3 rounded-xl font-black text-sm shadow-xl flex items-center gap-2 hover:bg-[#851a40] transition-all">
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddProject} className="p-8 space-y-6">
              ...
              <div className="flex justify-end gap-4 pt-8 border-t">
                ...
                <button type="submit" className="px-10 py-3 bg-[#9f224e] text-white rounded-xl font-black shadow-lg hover:bg-[#851a40] transform active:scale-95 transition-all">LƯU DỰ ÁN</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl animate-scale-in">
            <div className="p-8 border-b flex items-center justify-between">
              <div>
                ...
                <h2 className="text-2xl font-black text-slate-900 mt-2">{selectedProject.description}</h2>
              </div>
