...
const MemberHub: React.FC<MemberHubProps> = ({ projects }) => {
  ...
  const getMemberStats = (names: string[], roleLabel: string) => {
    return names.map(name => {
      ...
      return { name, role: roleLabel, total: totalInvolved, active: activeProjects, pmCount: projectsAsPM.length, designCount: projectsAsDesigner.length, poCount: projectsAsPO.length };
  ...
  const renderSection = (title: string, stats: ReturnType<typeof getMemberStats>, colorClass: string, icon: React.ReactNode) => (
    <div className="mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {stats.map((member) => (
          <div key={member.name} onClick={() => setSelectedMember(member.name)} className="bg-white/80 dark:bg-[#1e293b]/50 backdrop-blur-xl p-6 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700/50 hover:border-[#9f224e] hover:bg-white dark:hover:bg-[#1e293b]/70 hover:shadow-[0_0_20px_rgba(159,34,78,0.2)] transition-all group cursor-pointer active:scale-95 transform hover:-translate-y-1">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-white text-base shadow-lg overflow-hidden border-2 ${member.active > 0 ? 'border-[#9f224e] bg-[#9f224e]' : 'border-slate-300 dark:border-slate-600 bg-slate-400 dark:bg-slate-700'}`}>
                  {MEMBER_PROFILES[member.name]?.avatar ? <img src={MEMBER_PROFILES[member.name].avatar} alt={member.name} className="w-full h-full object-cover" /> : member.name.charAt(0).toUpperCase()}
                </div>
                <div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-fade-in" onClick={() => setSelectedMember(null)}>
          <div className="bg-white dark:bg-[#1e293b] w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 animate-scale-in flex flex-col md:flex-row" onClick={e => e.stopPropagation()}>
            <div className="w-full md:w-1/3 bg-slate-100 dark:bg-slate-900 flex items-center justify-center relative min-h-[300px] border-r border-slate-200 dark:border-slate-700">
               {MEMBER_PROFILES[selectedMember].avatar ? <img src={MEMBER_PROFILES[selectedMember].avatar} className="absolute inset-0 w-full h-full object-cover opacity-90" alt={selectedMember} /> : <div className="text-8xl font-black text-slate-300 dark:text-slate-800">{selectedMember.charAt(0).toUpperCase()}</div>}
            ...
            <div className="w-full md:w-2/3 p-10 flex flex-col justify-center bg-white dark:bg-[#1e293b]">
              <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">{MEMBER_PROFILES[selectedMember].fullName}</h2>
              <div className="space-y-5 text-sm">
                <div className="flex"><span className="w-32 font-bold text-slate-400 uppercase text-[10px] tracking-wider pt-1">Ngày sinh</span><span className="font-bold text-slate-700 dark:text-slate-200">{MEMBER_PROFILES[selectedMember].dob}</span></div>
                <div className="flex"><span className="w-32 font-bold text-slate-400 uppercase text-[10px] tracking-wider pt-1">Bộ phận</span><span className="font-bold text-slate-700 dark:text-slate-200">{MEMBER_PROFILES[selectedMember].department}</span></div>
                <div className="flex"><span className="w-32 font-bold text-slate-400 uppercase text-[10px] tracking-wider pt-1">Vị trí</span><span className="font-black text-[#9f224e] drop-shadow-[0_0_5px_rgba(159,34,78,0.8)]">{MEMBER_PROFILES[selectedMember].position}</span></div>
                <div className="flex"><span className="w-32 font-bold text-slate-400 uppercase text-[10px] tracking-wider pt-1">Email</span><a href={`mailto:${MEMBER_PROFILES[selectedMember].email}`} className="font-bold text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 hover:underline">{MEMBER_PROFILES[selectedMember].email}</a></div>
                <div className="flex"><span className="w-32 font-bold text-slate-400 uppercase text-[10px] tracking-wider pt-1">Ngày đi làm</span><span className="font-bold text-slate-700 dark:text-slate-200">{MEMBER_PROFILES[selectedMember].startDate}</span></div>
              </div>
